...
@billing_bp.route('/statements/<int:statement_id>/export.html', methods=['GET'])
@token_required
@tenant_isolation_required
def export_statement_html(statement_id):
    """Branded HTML statement for print/PDF pipeline."""
    s = Statement.query.get_or_404(statement_id)
    p = Patient.query.get(s.patient_id)
    name = f"{p.first_name} {p.last_name}".strip() if p else 'Patient'
    body = f"""<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>Statement {s.statement_id}</title>
<style>body{{font-family:system-ui,Segoe UI,sans-serif;max-width:720px;margin:2rem auto;padding:1rem;}}
h1{{font-size:1.25rem;}} .amt{{text-align:right;}}</style></head><body>
<h1>Account statement</h1>
<p><strong>Statement ID:</strong> {s.statement_id}<br>
<strong>Patient:</strong> {name} (ID {s.patient_id})<br>
<strong>Date:</strong> {s.statement_date}<br>
<strong>Due:</strong> {s.due_date}</p>
<table width="100%" border="0" cellspacing="0" cellpadding="6">
<tr><th align="left">Description</th><th class="amt">Amount</th></tr>
<tr><td>Total charges</td><td class="amt">{float(s.total_charges or 0):.2f}</td></tr>
<tr><td>Total payments / credits</td><td class="amt">{float(s.total_payments or 0):.2f}</td></tr>
<tr><td>Adjustments</td><td class="amt">{float(s.total_adjustments or 0):.2f}</td></tr>
<tr><th align="left">Balance due</th><th class="amt">{float(s.balance_due or 0):.2f}</th></tr>
</table>
<p style="color:#666;font-size:0.9rem">Generated by Clinic+ &mdash; print or save as PDF from your browser.</p>
</body></html>"""
