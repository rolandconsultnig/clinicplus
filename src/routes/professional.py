"""
Professional Credentialing API Routes
Comprehensive credentialing, privileging, and compliance management
"""
from flask import Blueprint, request, jsonify
from src.auth.jwt_manager import token_required, role_required
from src.models.user import db
from src.models.professional import (
    ProfessionalCredential, ProfessionalToken, 
    ProfessionalTokenRenewal, CredentialVerificationLog
)
...
@professional_bp.route('/credentials', methods=['POST'])
@token_required
@role_required(['physician', 'nurse', 'pharmacist'])
def upload_credential():
    ...
    try:
        ...
        if not provider_id:
            # Get provider from user account
            user_account = request.current_user
            provider = Provider.query.filter(Provider.user_account_id == user_account.id).first()
            if not provider:
        ...
        if data.get('document_content'):
            document_hash = hashlib.sha256(data['document_content'].encode()).hexdigest()
        ...
        credential = ProfessionalCredential(
            ...
            issuing_state=data.get('issuing_state'),
            issuing_country=data.get('issuing_country', 'USA'),
...
@professional_bp.route('/credentials/<int:credential_id>/verify', methods=['POST'])
@token_required
...
def verify_credential(credential_id):
    ...
    try:
        ...
        credential.verified_by = request.current_user.id
        credential.verified_at = datetime.utcnow()
        credential.verification_notes = data.get('notes')
        ...
        log = CredentialVerificationLog(
            ...
            verification_source=data.get('source', 'manual'),
            verification_result=data.get('result'),
            notes=data.get('notes'),
            action_taken='account_activated',
...
@professional_bp.route('/credentials/check-expiry', methods=['POST'])
@token_required
@role_required(['admin', 'system'])
def check_credential_expiry():
    """Check and update expired credentials (automated task)"""
    try:
        today = date.today()
        ...
        for credential in expired_credentials:
            ...
            provider = Provider.query.get(credential.provider_id)
            if provider:
                provider.employment_status = 'suspended'
                
                # Suspend user account
                user_account = UserAccount.query.filter(UserAccount.provider_id == provider.id).first()
                if user_account:
                    user_account.is_active = False
                    suspended_accounts.append(user_account.id)
            ...
            log = CredentialVerificationLog(
                ...
                provider_id=credential.provider_id,
                verification_type='expiry_check',
...
# Professional Tokens
@professional_bp.route('/tokens', methods=['POST'])
@token_required
@role_required(['physician', 'nurse', 'pharmacist'])
def purchase_professional_token():
    """Purchase professional token fee"""
    try:
        data = request.get_json()
        provider_id = data.get('provider_id')
        
        # Get provider from user account if not provided
        if not provider_id:
            user_account = request.current_user
            provider = Provider.query.filter(Provider.user_account_id == user_account.id).first()
            if not provider:
        ...
        # Token fee amount (configurable, default ₦5,000/month)
        token_fee = Decimal(str(data.get('token_fee_amount', 5000)))
        
        # Calculate subscription period (default 1 month)
        start_date = date.today()
        end_date = start_date + timedelta(days=30)
        
        token = ProfessionalToken(
            token_id=f"TOKEN-{uuid.uuid4().hex[:12].upper()}",
            provider_id=provider_id,
            user_account_id=request.current_user.id,
            token_fee_amount=token_fee,
...
@professional_bp.route('/tokens/<int:token_id>/pay', methods=['POST'])
@token_required
def pay_token_fee(token_id):
    """Pay professional token fee"""
    try:
        token = ProfessionalToken.query.get_or_404(token_id)
        data = request.get_json()
        
        token.payment_status = 'paid'
        token.payment_date = datetime.utcnow()
        token.payment_reference = data.get('payment_reference')
        token.status = 'active'
...
@professional_bp.route('/tokens/<int:token_id>/renew', methods=['POST'])
@token_required
def renew_token(token_id):
    """Renew professional token"""
    try:
        token = ProfessionalToken.query.get_or_404(token_id)
        data = request.get_json()
        
        # Create renewal
        renewal = ProfessionalTokenRenewal(
            token_id=token_id,
            renewal_date=date.today(),
            renewal_fee_amount=token.token_fee_amount,
            payment_status='pending',
            new_subscription_start=token.subscription_end + timedelta(days=1),
            new_subscription_end=token.subscription_end + timedelta(days=31)
        )
...
@token_required
def upload_document_comprehensive():
